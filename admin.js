...
// Valores padrão para o modo local (apenas para instalações sem backend)
const DEFAULT_USERNAME = 'admin';
const DEFAULT_PASSWORD = 'pibno2025';
...
async function handlePostSubmit(e) {
    ...
    try {
        ...
        // Preparar postData sem sobrescrever autor quando possível
        const postData = {
        ...
        // Se houver campos escondidos preenchidos pelo frontend, envie também
        const authorId = document.getElementById('post-author-id')?.value;
        const authorUsername = document.getElementById('post-author-username')?.value;
        const authorPreview = document.getElementById('post-author')?.value;
        if (authorId) postData.authorId = authorId;
        if (authorUsername) postData.authorUsername = authorUsername;
        if (authorPreview && !postData.author) postData.author = authorPreview;
...
function renderPostsList() {
    ...
    postsList.innerHTML = posts.map(post => {
        const dateText = new Date(post.date || post.createdAt).toLocaleDateString('pt-BR');
        let authorHtml = post.author || 'Anônimo';
        if (post.authorUsername) {
            authorHtml = `<a href="user.html?username=${encodeURIComponent(post.authorUsername)}" target="_blank">${post.author || post.authorUsername}</a>`;
        } else if (post.authorId) {
            authorHtml = `<a href="user.html?id=${post.authorId}" target="_blank">${post.author || 'Anônimo'}</a>`;
        }
        ...
        <div class="post-item">
            <div class="post-info">
                <h3>${post.title}</h3>
                <p>Por ${authorHtml} • ${dateText} • ${post.type === 'image' ? '📷 Imagem' : '🎥 Vídeo'}</p>
            </div>
...
function resetSystem() {
    if (confirm('⚠️ ATENÇÃO!\n\nIsso irá:\n• Criar/resetar o usuário admin padrão\n• Manter todos os posts e usuários existentes\n\nDeseja continuar?')) {
        ...
        const storedUsers = localStorage.getItem('pibno_users');
        let localUsers = storedUsers ? JSON.parse(storedUsers) : [];

        // Verificar se admin já existe
        const adminExists = localUsers.find(u => u.username === DEFAULT_USERNAME);
        ...
        if (!adminExists) {
            ...
            localStorage.setItem('pibno_users', JSON.stringify(localUsers));
            showAlert('login-alert', '✅ Usuário admin criado com sucesso! Altere a senha no primeiro acesso.', 'success');
        } else {
            // Reset da senha do admin
            const adminIndex = localUsers.findIndex(u => u.username === DEFAULT_USERNAME);
            localUsers[adminIndex].password = DEFAULT_PASSWORD;
            localStorage.setItem('pibno_users', JSON.stringify(localUsers));
            showAlert('login-alert', '✅ Senha do admin reinicializada. Altere-a no primeiro acesso.', 'success');
        }
    }
}
