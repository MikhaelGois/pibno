...
// Criar admin - EXECUTAR UMA ÚNICA VEZ
async function createFirstAdmin() {
  const result = await registerUser('admin@pibno.com', 'pibno2025', {
    username: 'admin',
    name: 'Administrador'
  });
  
  if (result.success) {
    // Agora vá no Firestore e mude o role para 'admin' e approved para true
    console.log('Admin criado! UID:', result.user.uid);
    console.log('Vá no Firestore e atualize o documento em users/' + result.user.uid);
    console.log('Defina: role = admin, approved = true');
  }
}

// Descomente para criar (execute apenas uma vez!)
// createFirstAdmin();
</script>
```
