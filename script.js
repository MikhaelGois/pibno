...
function createCarouselPostElement(post) {
    ...
    // Autor: linka para perfil público se houver username ou authorId
    let authorName = post.author || 'Anônimo';
    let authorHtml = `Por: ${authorName}`;
    if (post.authorUsername) {
        authorHtml = `Por: <a href="user.html?username=${encodeURIComponent(post.authorUsername)}">${authorName}</a>`;
    } else if (post.authorId) {
        authorHtml = `Por: <a href="user.html?id=${post.authorId}">${authorName}</a>`;
    }
    ...
    postDiv.innerHTML = `
        ...
        <div class="carousel-post-content">
            ...
            <p class="carousel-post-author">${authorHtml}</p>
...
function createBlogPostElement(post) {
    ...
    article.innerHTML = `
        ...
        <div class="blog-post-content">
            ...
            <p class="blog-post-author">${(post.authorUsername ? `Por <a href="user.html?username=${encodeURIComponent(post.authorUsername)}">${post.author || post.authorUsername}</a>` : (post.authorId ? `Por <a href="user.html?id=${post.authorId}">${post.author || 'Anônimo'}</a>` : `Por ${post.author || 'Anônimo'}`))}</p>
